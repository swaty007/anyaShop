...
$advancedTemplate = !get_post_meta($post_id, 'advanced', true);
$test_lesson = !!get_post_meta($post_id, 'test_lesson', true);
$popup_show = !!get_post_meta($post_id, 'popup_show', true);
...
$type_education = get_post_meta($post_id, 'type_education', true);
$about = get_post_meta($post_id, 'about', true);
$about_card = get_post_meta($post_id, 'about_card', true);

$youtube = get_post_meta($post_id, 'youtube', true);
$youtube_title = get_post_meta($post_id, 'youtube_title', true);
$youtube_content = get_post_meta($post_id, 'youtube_content', true);
$for_who_course = get_post_meta($post_id, 'for_who_course', true); //repeater advanced
...
$education_formula_title = get_post_meta($post_id, 'education_formula_title', true);
$education_formula = get_post_meta($post_id, 'education_formula', true); //repeater
$details_title = get_post_meta($post_id, 'details_title', true);
$details_html = get_post_meta($post_id, 'details_html', true);
$details_items = get_post_meta($post_id, 'details_items', true);
...
$reviews = get_post_meta($post_id, 'review', true);
$teachers_title = get_post_meta($post_id, 'teachers_title', true);
...
<section class="main main--course">
    <img src="<?= get_template_directory_uri(); ?>/images/bg/hands.png"
         class="main__bg-image main__bg-image--course">
    <div class="main__overlay"></div>
    <div class="container main__container main__container--flex">
        <div class="main__block">
            <p class="text-md fromRight">
                <?= $type_education;?>
            </p>
            <h1 class="title text-bold main__title main__title--course fadeFromLeft">
                <?php the_title(); ?>
            </h1>
            <div class="text-md main__subtitle main__subtitle--lg fromRight">
                <?php the_excerpt(); ?>
            </div>
            <div class="main__btns main__btns--hide fromBottom" >
                <a href="#form" class="btn main__btn   "><?php pll_e("Забронировать место"); ?></a>
                <?php if($test_lesson):?>
                <a href="<?= get_permalink(pll_get_post(get_page_id_by_template('page-consult.php')));?>?target_id=<?= empty($lang_post_id) ? $post_id : $lang_post_id;?>&trial=1"
                   class="btn btn-nobg main__btn main__btn-nobg ">
                    <?php pll_e("Получить запись занятия"); ?></a>
                <?php else:?>
                    <a href="<?= get_permalink(pll_get_post(get_page_id_by_template('page-consult.php')));?>?target_id=<?= empty($lang_post_id) ? $post_id : $lang_post_id;?>&counsult=1"
                       class="btn btn-nobg main__btn main__btn-nobg ">
                        <?php pll_e("Хочу консультацию"); ?></a>
                <?php endif;?>
            </div>
        </div>
        <div class="main__table fromRight">
            <div class="main__table-tr">
                <p class="main__table-label main__table-label--first"><?php pll_e("КОГДА"); ?></p>
                <p class="text-md main__table-title text-bold"><?php pll_e("Старт"); ?></p>
                <?php if (have_rows('date')):?>
                    <?php while (have_rows('date')): the_row();?>
                        <p class="main__table-td"><?php the_sub_field('date') ?></p>
                    <?php endwhile; ?>
                <?php else:?>
            ...
            <div class="main__table-tr">
                <p class="main__table-label main__table-label--second"><?php pll_e("СКОЛЬКО"); ?></p>
                <p class="text-md main__table-title text-bold"><?= $lessons; ?> <?php pll_e("занятий"); ?></p>
                <p class="main__table-td">2-3 раза в неделю</p>
                <p class="main__table-td">с 20:00 до 23:00 по МСК</p>
            </div>
            <div class="main__table-tr">
                <p class="main__table-label main__table-label--third"><?php pll_e("ЦЕНА"); ?></p>
                <?php if ($discount_price): ?>
                    <p class="main__table-title text-bold text-md main__table-title--crossed"><?= $cost; ?> <?= $currency; ?></p>
                    <p class="main__table-title text-bold text-md"><span id="current_price"><?= $discount_price; ?></span> <?= $currency; ?></p>
                <?php else: ?>
                    <p class="main__table-title text-bold text-md"><span id="current_price"><?= $cost; ?></span> <?= $currency; ?></p>
                <?php endif; ?>
                <?php if ($ismonth): ?>
                    <p class="main__table-td main__table-td--added"><?php pll_e("Доступна оплата частями"); ?></p>
                <?php endif; ?>
            </div>
        </div>
        <div class="main__btns main__btns--mob">
            ...
            <?php if($test_lesson):?>
                <a href="<?= get_permalink(pll_get_post(get_page_id_by_template('page-consult.php')));?>?target_id=<?= $post_id ;?>&trial=1"
                   class="btn btn-nobg main__btn main__btn-nobg fromBottom"><?php pll_e("Получить запись занятия"); ?></a>
            <?php else:?>
                <a href="<?= get_permalink(pll_get_post(get_page_id_by_template('page-consult.php')));?>?target_id=<?= empty($lang_post_id) ? $post_id : $lang_post_id;?>&counsult=1"
...
<?php if(get_field('youtube_const') == 1):?>
<section id="youtube" class="youtube">
    <div class="container">
    <h2 class="third-title text-bold text-center youtube_header">
                   <?= $youtube_title; ?>
               </h2>
       <div class="youtube__wrap">
           <div class="youtube__item">
                ...
                $imageUrl = get_field('youtube_link_for_image');
                parse_str( parse_url( $imageUrl, PHP_URL_QUERY ), $youtubeArray );
                $videoId = $youtubeArray['v'];
                $imageUrl = "https://img.youtube.com/vi/$videoId/mqdefault.jpg";?>
                <div class="youtube_prev" onclick="myFunction();return false;">
                    <div style="background:url(<?= $imageUrl; ?>) center no-repeat; background-size: contain;" class='youtube__img' > </div>
                    
                    <img  class='youtube_btn'src="<?php echo get_template_directory_uri()?>/images/youtubeBTN.svg" alt="">
               
                </div>
                <div id='player' class='you_player' style='display:none;'>

                </div>
               
           </div>
           <div class="youtube__item">
               <h2 class="third-title text-bold text-center">
                   <?= $youtube_title; ?>
               </h2>
               <div class="wordpress__content">
                   <?= get_field('youtube_content');?>
               </div>
           </div>
       </div>
    </div>
...
<?php endif;?>
<?php  if(get_field('banner_const')):?>
<section id="banner" class="banner">
    <div class="container">
        <h2 class="third-title text-bold text-center">
            <?php pll_e("Для кого этот курс:"); ?>
        </h2>
        <?php if ($advancedTemplate): ?>
            <div class="banner__header">
                <?php $count = 0;
                while (have_rows('for_who_course')): the_row(); ?>
                    <div class="banner__header-item">
                        <?php if ($count === 0): ?>
        ...
        <div class="banner__container">
            <?php if ($advancedTemplate): ?>
                <div class="banner__block fromBottomHalf banner__block--photos">
                    <div class="banner__block-green"><img class="banner__block-photo"
                                                          src="<?= get_template_directory_uri(); ?>/images/people/man.png"
                                                          alt=""></div>
                    <div class="banner__block-purple"><img class="banner__block-photo"
                                                           src="<?= get_template_directory_uri(); ?>/images/people/woman.png"
                                                           alt=""></div>
                    <div class="banner__block-pink"><img class="banner__block-photo"
                                                         src="<?= get_template_directory_uri(); ?>/images/people/girl.png"
                                                         alt=""></div>
                </div>
            
            <?php elseif($youtube_in_who):?>
            
            
            <?php else: ?>
                <div class="banner__block banner__block--photos">
                    <div class="banner__block-green"><img class="banner__block-photo"
                                                          src="<?= get_template_directory_uri(); ?>/images/people/man1.png"
                                                          alt=""></div>
                    <div class="banner__block-purple"><img class="banner__block-photo"
                                                           src="<?= get_template_directory_uri(); ?>/images/people/woman1.png"
                                                           alt=""></div>
                    <div class="banner__block-pink"><img class="banner__block-photo"
                                                         src="<?= get_template_directory_uri(); ?>/images/people/girl.png"
                                                         alt=""></div>
            ...
            <div class="banner__table">
                <div class="fadeFromLeft">
                    <?= $about; ?>
                </div>
                <a href="<?= get_permalink(pll_get_post(get_page_id_by_template('page-consult.php')));?>?target_id=<?= empty($lang_post_id) ? $post_id : $lang_post_id;?>&consult=1"
                   class="btn btn-nobg banner__table-btn fadeFromLeft">
...
<?php endif; ?>
<section id="program" class="program section--blue">
    <div class="container">
        <div class="program__top">
            <h2 class="third-title program__top-title text-bold">
                <?= $how_you_learn_title ?>
            </h2>
            <a href="#plan" class="btn btn-nobg"><?php pll_e("Программа курса"); ?></a>
        </div>
        <div class="program__info">
            <?php $count = 0;
            while (have_rows('how_you_learn')): the_row(); ?>
                <div class="program__item fadeFromLeft">
                    <div class="program__item-circle fadeFromLeft">
                        <?php if ($count === 0): ?>
                    ...
                    <div>
                        <p class="program__item-title text-bold"><?= get_sub_field('title') ?></p>
                        <p>
                            <?= get_sub_field('text') ?>
                        </p>
                    </div>
...
<!-- <section id="banner2" class="banner banner--dotted banner--col section--brand">
    <div class="container banner__container">
        <img src="<?= get_template_directory_uri(); ?>/images/people/girl1.png" class="banner__photo">
        <p class="second-title banner__title banner__title--lg text-bold">
            <?= $slider_title;?>
        </p>
        <ul class="banner__list">
            <?php while (have_rows('slider_items')): the_row(); ?>
                <li class="banner__list-item"><?= get_sub_field('text') ?></li>
            <?php endwhile; ?>
        </ul>
    </div>
...
<section    class='after_course section--blue'>

        <div class="container">
            <h2><?php the_field('after_course_title')?></h2>

            
           <?php if( have_rows('after_course_array') ): ?>
                <ul>

          <?php  while( have_rows('after_course_array') ) : the_row(); ?>
                 <li> <?php echo get_sub_field('after_course_item');?></li> 
...
<section id="benefites-list" class="benefites-list benefites-list--course">
    <div class="container">
        <h2 class="title text-center benefites-list__title">
            <?= $education_formula_title;?>
            <?php //pll_e("Формула эффективного обучения ITEA:"); ?>
        </h2>
...
<section id="skyeng" class="skyeng"> 
   <div class="container">
   <div class="skyeng_wraper">
        <div class="skyeng_label"><p>Бонус для наших студентов</p></div>
        <div class="skyeng_wrap">
                   <div class="skyeng_alltext">
                        <h2 class="skyeng_title"><strong>Практика английского</strong> в самой большой онлайн-школе Европы <strong>SkyEng</strong></h2>
                        <div class="skyeng_text"><p><strong>3 урока</strong> английского языка <strong>в подарок</strong></p></div>
                   </div>
                    <a href="https://go.skyeng.ru/itea" target="_blank" class="skyeng_link">Получить подарок</a>
        </div>
        <div class="skyeng_subtext"><p>*Для получения подарка, активируйте промокод <strong>ITEA3</strong> в личном кабинете при первой покупке от 8 занятий</p></div>
        </div>
   </div>
...
<section id="details" class="details">
    <div class="container details__container">
        <div class="details__block">
            <p class="third-title text-bold details__title">
                <?= $details_title;?>
            </p>
            <p class="fifth-title details__subtitle">
                <?= $details_html;?>
            </p>
            <a href="#form" class="btn details__btn details__btn--hide"><?php pll_e("Записаться на курс"); ?></a>
        </div>
        <div class="details__block">
            ...
            <?php while (have_rows('details_items')): the_row();
                ...
                <div class="details__item">
                    <div class="details__item-top">
                        <div class="details__item-column">
                            <h4 class="text-bold"><?php the_sub_field('title'); ?></h4>
                            <div class="details__item-subtitle"><?php the_sub_field('description'); ?></div>
                        </div>
                        <div class="fourth-title text-bold">$<?= $price;?>+</div>
                    </div>
                    <div class="details__progress">
                        <div class="details__progress-done"
                             style="width:<?=$price * 100 / $max_price ;?>%;"></div>
                    </div>
                </div>
            <?php endwhile; ?>
            <a href="#form" class="btn details__btn details__btn--mob"><?php pll_e("Записаться на курс"); ?></a>
...
<section id="plan" class="plan">
    <div class="container">
        <p class="third-title text-bold plan__title text-center">
        ...
        <div class="plan__show">
            ...
            while (have_rows('education_plan')): the_row(); ?>
                <div class="page__item page__item--bordered page__item--narrow fromBottom">
        ...
        <div class="plan__hidden" style="display:none;">
            <?php if($count >= 10):?>
            <?php while (have_rows('education_plan')): the_row(); ?>
                <div class="page__item page__item--bordered page__item--narrow">
                    <div class="page__item-header">
                        <p class="sixth-title text-medium"><?php the_sub_field('title') ?></p>
                        <div class="btn-plus page__item-btn-plus"></div>
                    </div>
                    <div class="page__item-body">
                        <?php the_sub_field('text') ?>
        ...
        <?php if ($count > 10): ?>
            <button class="btn btn-nobg plan__btn" onclick=""><?php pll_e("Детальнее"); ?></button>
        <?php endif; ?>
    </div>
</section>
<?php if (!empty($reviews)): ?>
    <section id="feeds-list" class="feeds-list feeds-list--nopadd section--purple">
        <div class="third-title text-bold text-center"><?php pll_e("Отзывы студентов"); ?></div>
        <div class="container">
            <div class="lazy slider feeds-wrap">
                <?php foreach ($reviews as $review): ?>
                    <div class="feeds feeds--wide">
                        <img src="<?= get_the_post_thumbnail_url($review); ?>" class="feeds__photo">
                        <div class="feeds__main">
                            <h3 class="feeds__title text-bold">
                                <?= get_the_title($review); ?>
                            </h3>
                            <div class="text-md"><?= get_the_excerpt($review); ?></div>
                            <h4 class="text-center feeds__text">
                                <?= apply_filters('the_content', get_post_field('post_content', $review)); ?>
                            </h4>
                        </div>
                    </div>
                <?php endforeach; ?>
...
<?php if (!empty($teachers)): ?>
    <section id="teachers-list" class="teachers-list teachers-list--padding">
        <div class="container">
            <div class="teachers-list__top teachers-list__top--flex">
                <p class="title teachers-list__title teachers-list__top--flex__title">
                    <?= $teachers_title;?>
                    <?php //pll_e("Кто преподаватели?"); ?>
                </p>
                <p class="text-md teachers-list__subtitle">
            ...
            <?php if (count($teachers) > 1):?>
                <div class="teachers-list__wrap teachers-list__wrap--center">
                    <?php foreach ($teachers as $teacher): ?>
                        <div class="teacher">
                            <div class="teacher__photo">
                                <img src="<?= get_the_post_thumbnail_url($teacher);?>" />
                                <p class="teacher__photo-info">
                                    <?= get_post_meta($teacher, 'short_description', true);?>
                                    <!--                               --><?php //the_content();?>
                                </p>
                            </div>
                            <p class="fourth-title teacher__title text-bold"><?= get_the_title($teacher);?></p>
                        ...
                        <div>
                            <div class="teachers-list__item">
                                ...
                                <div class="teachers-list__item-info">
                                    <p class="fourth-title teachers-list__item-title text-bold"><?= get_the_title($teacher); ?></p>
                                    <div class="sixth-title teachers-list__item-subtitle"><?= get_the_excerpt($teacher); ?></div>
...
<?php endif; ?>
<form id="form_courses" class="form section--brand">
    <div class="container">
                ...
                <div class="form__info">
                    ...
                    <div class="form__block">
                        ...
                        <div class="form__main-content">
                            ...
                            <div class="text-md form__block-text text-light-purple">
                                Выбирай удобную рассрочку на 4 месяца и получай знания с комфортом.                        </div>
                        </div>
                    </div>
                </div>
...
<!---->
<!--        </div>-->
<!--        <p class="third-title text-bold text-center fromBottomHalf">-->
<!--            --><?php //pll_e("Успей забронировать свое место в группе"); ?>
<!--        </p>-->
<!--        <p class="text-md text-center form__subtitle fromBottomHalf">-->
    ...
    <section id="card-list" class="card-list">
        <div class="container card-list__container">
            ...
            <div class="card-list__top">
                <div class="title-md text-bold card-list__top-title">
                    <?php pll_e("Тебе могут подойти эти курсы:"); ?>
                </div>
                <a href="<?= get_permalink(pll_get_post(get_page_id_by_template('page-schedule.php'))); ?>"
                   class="btn btn-nobg"><?php pll_e("Расписание курсов"); ?></a>
            </div>
            <div class="card-wrap">
...
<?php endif; ?>
<?php if (!empty($faqs)): ?>
<section id="faq" class="plan plan--prof">
    <div class="container">
        <div class="third-title text-bold plan__title text-center">
            <?php pll_e("Отвечаем на самые часто задаваемые вопросы");?>
        </div>
        <div class="plan__show">
            <?php foreach($faqs as $faq):?>
                <div class="page__item page__item--bordered page__item--narrow fromBottom">
                    <div class="page__item-header">
                        <div class="sixth-title text-medium"><?= get_the_title($faq) ?></div>
                        <div class="btn-plus page__item-btn-plus"></div>
                    </div>
                    <div class="page__item-body">
                        <?= get_the_excerpt($faq) ?>
...
<script>
            ...
            function myFunction() {
                document.getElementById("player").innerHTML = "<div id='player'></div>";
